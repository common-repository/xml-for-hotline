...
final class XFHU_Error_Log {
	...
	public function __construct($text_to_log, $log_dir_name = XFHU_PLUGIN_UPLOADS_DIR_PATH) {
	...
	protected function keeplogs_status() {
		if (is_multisite()) {
			$v = get_blog_option(get_current_blog_id(), 'xfhu_keeplogs');
		} else {
			$v = get_option('xfhu_keeplogs');
		}	
		if ($v === 'on') {
			return true;
		} else {
